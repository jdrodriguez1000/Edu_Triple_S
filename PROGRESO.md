...
**Última actualización:** 2026-08-23 (sesión 106 — NIVEL 8: **ARRANCA EL BLOQUE D Y D.1 QUEDA HECHA: `compartida.py`, 28 pruebas, $0,000000 el día entero.** 🎲 Primero se selló la apuesta y se commiteó sola, antes de la primera línea de código: **once sesiones seguidas con ese orden**, y esta vez sirvió para algo incómodo — **dos de las seis apuestas fallaron, y las dos fallaron mejor de lo que habrían acertado.** 🚨 **EL HALLAZGO DEL DÍA NO LO ESTABA MIRANDO NINGUNA DE LAS SEIS**, igual que en la 97: `06b/memoria.py` promete por escrito que un archivo dañado NO se borra —*«es la única evidencia de qué pasó»*— y **la promesa dura tres líneas**. `cargar_memoria()` la cumple; `guardar_dato()` la llama, recibe el `[]` que significa *«no pude leer»*, le añade el dato nuevo y **reescribe el archivo entero**. Medido: 3 datos sanos, el archivo cortado a la mitad, UNA escritura después → archivo válido con 1 dato, evidencia borrada, y de vuelta un **`(True, "guardado")`** en verde. 🔑 **Nadie mintió: las dos funciones hacen lo que dice su comentario. Lo que no existía era el comentario que las mirara juntas** — y esa contradicción no vive en ninguna de las dos, así que no hay dónde leerla → `LM.85`. 📊 **La trampa que el temario le puso a D.1, medida y con su control al lado:** dos workers guardando a la vez **pierden el 49,5 % de lo que escriben y dejan el archivo PERFECTAMENTE VÁLIDO** — 0 % de archivos rotos, cero excepciones, cero avisos. Con 12 hilos, 81,9 % perdido y 24,5 % de archivos rotos. Con el arreglo puesto, **0/480 perdidos** en las cuatro filas. 🔑 **Un `.jsonl` roto grita; un estado pisado calla** — y por eso el candado de B.2 no se podía copiar tal cual: allá el síntoma es un archivo ilegible, aquí uno legible que miente → `LM.86`. ⭐ **La apuesta 3 salió mejor de lo apostado y es el titular técnico:** una versión con candado de hilos + relectura dentro + escritura atómica —**todo lo que una revisión de código llamaría correcto**, y pasa las 18 pruebas de hilos sin despeinarse— **pierde el 80 % entre cinco procesos, MÁS que no poner nada (76,7 %)**. Con candado de archivo, 0 %. 🔑 **Ese modo de fallo no se ve leyendo el código ni corriendo las pruebas que cualquiera escribiría: solo se ve lanzando dos `python`** → `LM.87`, y tiene fecha, porque el bloque E son agentes programados y su primera pregunta escrita es *«¿qué pasa si se dispara dos veces?»*. 🔴 **La apuesta 1 FALLÓ y trajo el dato mejor:** aposté que dos candados sobre un mismo archivo no romperían nada con líneas cortas, y con **60 bytes se pierden 16 registros de 2400**; con un solo candado, **0 de 2400 en las tres tallas**. Y el síntoma dominante **no son líneas rotas, son líneas que desaparecen enteras** — el comentario de `orquestador.py:206` predice el ruidoso y ocurre el mudo. 🟡 **La apuesta 2 ganó exactamente donde la puse y era incompleta un paso más allá:** dije *«sin romper el archivo»* sin escribir a cuántos hilos, y a 12 se rompe el 24,5 %. **Era verdadera en su escala y falsa fuera de ella, y la escala no la escribí yo.** 🪤 **Y salió un cuarto caso que no aposté: «atómico» no quiere decir «siempre se puede».** En Windows `os.replace()` **niega** el renombrado si otro tiene el destino abierto — `PermissionError [WinError 5]`, **26 de 60 procesos caídos**, y los 16 clasificados eran ese error sin una sola excepción de otra clase. En Linux esa prueba habría estado verde para siempre. ⚠️ **Tres veces en el día la misma forma, y la tercera fue mía:** al ponerle reintentos al `os.replace()` los 26 caídos pasaron a **0** y las pérdidas **no se movieron** — **el arreglo se llevó el síntoma ruidoso y dejó intacto el silencioso** → `LM.88`. ✅ **La apuesta 5 también ganó: la política de memoria de la sesión 18 no sobrevivió al segundo escritor.** Cada dato lleva ahora `quien`, y el desalojo tiene **reserva** —no se saca el último dato de un worker si otro tiene más de uno—, que es `RepartoDeEntrada` de C.2 aplicado a renglones en vez de a dólares; **que la misma forma sirva para las dos cosas es la señal de que era estructura**. Su límite queda probado en `P13`: si los ocho son de ocho workers, la reserva no se puede cumplir y sale el más viejo. 📌 **`GUIDE.md` §6.e lleva ya la fila de `compartida.py`, escrita en el mismo commit** — es el segundo archivo que entra el mismo día, y el caso fácil porque **no tiene modo de pago que declarar**; aun así entra, porque la lista no dice «qué cuesta», dice «qué pasa si lo corro sin mirar». 📌 Se corrieron además `presupuesto.py` (**58**) y `traza.py` (**46**), **verdes las dos** — no se tocó ningún `.py` existente, pero se citan sus líneas. ➡️ **La 107 va a D.2 — skills compartidas**: el menú entero se paga en cada worker, apoyándose en `skills.py` del 6b. 🎲 Y la primera cosa vuelve a ser sellar la apuesta y commitearla. 📎 Deudas nuevas de D.1, todas con dueño y ninguna bloqueante: el candado rancio de 30 s es **un supuesto escrito y no medido** (media), `cargar()` no pide el candado (baja), **ningún worker llama a `recordar()` todavía** — D.1 midió la plomería, que es donde estaba el fallo (media) —, y `presupuesto.py:823` sigue apuntando dos candados al mismo archivo, que hoy no muerde porque esas pruebas no corren en paralelo (media, **medido, no supuesto**). 📎 Y siguen vivas las del bloque C: el contrato de una CADENA (`LM.72`), `profundidad.py:213`, la apuesta 4 de C.6 sin medir de verdad, el nombre del archivo de registro que lleva un solo modelo, `agente.costo()` viva en `router.py:298` y `supervisor.py:258`, y `recursion.py` sin pasar la capa hacia abajo.)
