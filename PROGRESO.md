...
**Última actualización:** 2026-08-24 (sesión 109 — NIVEL 8: **E.2 CIERRA, Y CON ELLA EL BLOQUE E ENTERO: `avisador.py`, 38 pruebas, $0,000000 — cuarta sesión seguida a cero, y 6 de 6 apuestas.** 🎲 Primero se selló la apuesta y se commiteó sola: **catorce sesiones seguidas** con ese orden. Cinco escalones, cada uno destapando lo que el anterior daba por hecho. 🚨 **EL HALLAZGO DEL DÍA: la regla de aviso más razonable del mundo, corrida sobre los 1 468 renglones REALES del nivel, manda 163 avisos y NI UNO ERA NOTICIA — 100 % de falsos positivos.** Y la regla no está mal: sus tres condiciones son las únicas señales de problema que ese registro sabe dar. 🔑 **Un `ok:False` con motivo `presupuesto` es IDÉNTICO renglón por renglón cuando lo provocó una prueba y cuando arruinó la corrida de un cliente. No se distinguen porque NO SON DISTINTOS:** lo distinto es el mundo alrededor, y de ese mundo solo tiene noticia el que estaba dentro. ⭐ **La gravedad no es una propiedad del renglón: es del MOMENTO en que se escribió** (`LM.92`), y por eso solo se escribe entonces — rellenarla hoy sobre lo de ayer sería inventármela (`LM.65`), y los 1 468 pagados no la tendrán jamás. ⚠️ **Y el atajo que parece funcionar es la trampa:** el 95 % del ruido sale de un archivo llamado `registro_pruebas_gratis.jsonl`, así que UNA línea lo mata — y no sirve, porque **ese filtro no lee un campo, lee un NOMBRE DE ARCHIVO**: la separación entre lo real y lo provocado era un accidente de cómo se guardó. 📌 Y la columna «¿era noticia?» **la puse yo a mano**, dicho en el código y no escondido: el único que sabe si un renglón era noticia es alguien que YA SABÍA LA RESPUESTA. ✅ **La apuesta 3 salió con su control:** el turno MUDO no se puede avisar con el registro **por construcción** —el que no corre no escribe—, y hacen falta **dos entradas de naturaleza distinta**: el registro lo escribe el que trabaja, el calendario el que prometió. `P23` lo hace falsificable: alargar el calendario mueve los mudos de 1 a 3 **sin tocar un renglón**. 🚨 Y de ahí sale dónde vive el código: **el avisador no puede correr dentro del proceso que vigila** — el que se murió a las 3:04 no manda el aviso de que se murió. ✅ **La apuesta 4 salió, y la salida también:** con la red caída, el `try/except: pass` manda 0 avisos, 0 errores, ningún rastro, y sale con código 0 — **el avisador no se rompió: SE CALLÓ**. Y el segundo `try` no arregla nada porque **viajaría por el mismo canal caído**. ⭐ **LA ALARMA LA MANDA EL QUE FALLA; EL LATIDO LO ECHA DE MENOS EL QUE ESCUCHA** (`LM.93`) — el latido invierte quién actúa, y es el MUDO aplicado al vigilante: se comprueba una ausencia contra un ritmo prometido. 🚨 Y tiene que **confesar, no solo respirar**: sin anotar los envíos fallidos, un latido es un `except: pass` con mejor prensa. ⚠️ Y no cierra la cadena, la alarga: **termina fuera, en algo que no controlas.** ✅ **La apuesta 5, medida ANTES de arreglar nada:** `disparador.py` con una prueba en rojo salía con **código 0** y `router.py` con **1** — mismo nivel, misma mano, misma semana, el mismo fallo, y uno gritaba y el otro decía que todo fue bien. 🔑 Y era justo el módulo pensado para correr SIN NADIE DELANTE. ⭐ **El aviso más barato del mundo ya estaba ahí y se tiraba en la última línea: no hacía falta escribir un canal, hacía falta no tirar el que había.** Arreglado el mismo día, después de medir. 🎁 **Y EL MEDIDOR TUVO DOS VECES EL BICHO QUE VENÍA A MEDIR:** dio 1 en los dos módulos, y las dos veces era un reventón (`NameError`, `ModuleNotFoundError`), no una prueba en rojo. 🔑 **UN CÓDIGO 1 NO DICE «FALLÓ LA PRUEBA»: DICE «ALGO PASÓ»** (`LM.95`), y lo cazó pedirle el `stderr` en vez de creerle el número. ⭐ **Y EL ARREGLO MATÓ A LA PRUEBA QUE LO MIDIÓ:** `P31` medía el 0 y se puso roja al instante. **Una prueba que describe el estado roto muere el día que lo arreglas** (`LM.94`); lo que sobrevive es un detector de la FORMA —`P35` lee con `ast` el `__main__` de los 14 módulos con suite y pregunta si alguno tira el resultado de sus pruebas: hoy ninguno—, y `P36` lo obliga a morder sobre un archivo torcido (`LM.13`). ✅ **La apuesta 1 se cerró con una CUENTA, no con una impresión:** 5 sentencias emiten contra 69 que deciden = **6,8 %**, dentro del «menos del 10 %» apostado. ⭐ *«No tenemos alertas» casi nunca significa que falte el emisor.* ✅ **Y la apuesta 6 salió con su trampa cumplida:** $0,000000, y las 38 pruebas miden **el emisor** y ni una **el receptor** — el hueco del título del bloque, dicho donde no puede disfrazarse de conclusión. 📌 **`GUIDE.md` §6.e lleva ya la fila de `avisador.py`, en el mismo commit** — quinto archivo seguido que entra el día que nace. 📌 Y `LESSONS.md` suma `LM.92`, `LM.93`, `LM.94` y `LM.95`. ➡️ **La 110 abre el BLOQUE F — Medir y decidir**, que arranca por `F.1`: la rúbrica de dos capas, el campo que dice **quién falló**. 🎲 Y lo primero vuelve a ser sellar la apuesta y commitearla. 📎 Deudas nuevas de E.2: **ningún módulo escribe el campo `entorno`** (alta, no bloqueante — sin él cualquier avisador vuelve al 100 % de falsos positivos), **el latido sin cablear a ningún proceso real** (media), **nadie escucha el latido** (media), **los escalones 2 y 3 medidos sobre demos fabricadas** (baja) y **`_copia_con_prueba_rota()` escribe dentro de la carpeta del nivel** (baja). 📎 Siguen vivas las de E.1 —`profundidad.py:594` con nombre fijo, los cinco `open(REGISTRO, "a")` sin candado de disco (alta), el disparador sin cablear, el coste del disparo doble multiplicado— y las de D.2, D.1 y el bloque C. ✅ Pagada la de `skills_compartidas.py:586`: `avisador.py` pone su `reconfigure()` al importar, no en `__main__`.)
