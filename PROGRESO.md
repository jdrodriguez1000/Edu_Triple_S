...
**Última actualización:** 2026-08-24 (sesión 110 — NIVEL 8: **ABRE EL BLOQUE F y F.1 CIERRA: `atribuidor.py` + `rubrica_duelo.md` v2, 24 pruebas, $0,000000 — quinta sesión seguida a cero, y 5 de 6 apuestas enteras.** 🎲 Primero se selló la apuesta y se commiteó sola: **quince sesiones seguidas** con ese orden. 🔑 **LA FORMA DE F.1 ES «DOS INSTRUMENTOS, NO UNO», y el nudo estaba DECLARADO CUATRO DÍAS ANTES en `rubrica_duelo.md:50` sin que nadie lo cobrara:** la rúbrica prohíbe que el juez sepa cuántas capas hubo —si ve workers, califica el esquema en vez de la respuesta— y el campo «quién falló» **exige** saberlas. Las dos correctas, y no caben en la misma cabeza. ⭐ **A un instrumento al que le tapas los ojos a propósito no le puedes pedir además que señale con el dedo** (`LM.96`) — así que la venda **no se levantó**: el juez ciego (capa 1) sigue diciendo QUÉ casilla falló, y `atribuidor.py` (capa 2) lee la traza de C.1 y dice DE QUIÉN es. 📌 Y la capa 2 **no pregunta a ningún modelo, y no por ahorrar**: sería un TERCER OPINANTE donde hace falta un TESTIGO. `P22` lee el propio archivo y se pone roja si aparece una llamada a la API — sin ella la apuesta 1 dejaría de ser cierta **en silencio**. 🚨 **EL HALLAZGO DEL DÍA ES LO QUE EL INSTRUMENTO NO HA PODIDO MEDIR: sobre las 7 corridas reales, 63 casillas atribuidas a $0,00 —54 `ok`, 7 `esquema:presupuesto`, 2 `esquema:contrato`— y `orquestador` salió CERO VECES.** El estado que este archivo existe para poder marcar **no se ha visto morder ni una vez en el mundo**: muerde en `P7` y `P8`, y esas dos corridas **las fabriqué yo**. 🔑 No es un fallo del instrumento: **el defecto necesita dos capas para existir y las dos capas todavía no han corrido el duelo.** F.3 es la primera vez que ese número puede dejar de ser cero — y si sigue en cero, **eso también es un hallazgo**. ⭐ Y estaba escrito en la **apuesta 6 ANTES de medir**, palabra por palabra: `LM.13` con fecha puesta por adelantado, que es la única forma de que no se lea como excusa después. 🚨 **EL `ok` DE CADA CAPA NO ES UN VEREDICTO: ES UNA DECLARACIÓN DE LA PARTE** (`LM.98`). De 60 `worker_fin`, cinco dicen `ok:False` — y **los cinco traen el contrato COMPLETO y `faltan: []`**: ninguno falló por el dato, el presupuesto los cortó **después** de que ya tenían la respuesta (en una corrida, los tres a la vez, con el tope en `$0.00`). Un atribuidor que le creyera al `ok` **señalaría al que sí hizo el trabajo, cinco veces**. Y el otro lado es simétrico y peor: **20 de 20 `orquestador_fin` dicen `ok:True`** mientras la sesión 95 midió que al menos una vez enrutó el euro al worker del dólar. 🎁 **EL CRUCE ACIERTA EN EL HECHO Y SE EQUIVOCA EN EL CULPABLE:** «¿el encargo corresponde a la tarea?» salta en 3 corridas —al worker del CAD le llega una cadena de tres eslabones donde la tarea pedía una conversión— y **quien lo torció NO fue el orquestador**: está fijo a mano en `presupuesto.py:406`, `ENCARGOS_DESIGUALES`, puesto a propósito por C.2. 🔑 **El registro guarda `encargo` pero NO quién lo escribió** — es `LM.92` de ayer con otra ropa. 🎁 **Y un caso donde los dos hicieron bien su parte y la respuesta correcta se tiró a la basura:** el worker del CAD acertó (`1.000 CAD = 2.219.774 COP`), el contrato de A.3 **solo tiene sitio para UNA conversión**, guardó el último eslabón, y el orquestador **se negó a publicar una cifra que no podía respaldar** — que es lo correcto. De ahí sale que **`esquema` no era un estado: eran TRES** (presupuesto, contrato, aislamiento). 🟡 **La apuesta 2 salió partida por la mitad y la mitad fallada es la mejor:** ✅ sí hace falta `no_atribuible`, 🔴 pero **NO cae sobre `C4-DOLAR`** — C4 sí es atribuible, al **esquema**, porque el aislamiento de A.4 impide a los dos lados verlo. `no_atribuible` quedó para algo mejor: **el juez dice `FALLA` y la traza no ve nada torcido**, un desacuerdo entre dos testigos que la rúbrica ahora está **obligada a confesar** en vez de promediar (`LM.66`). Aposté 4 estados: salieron **7**. 🟢 **La apuesta 5 salió y su motivo resultó más fuerte que ella:** las 11 casillas siguen siendo 11, porque **el contendiente A YA PASÓ por el instrumento y su juicio está pagado** ($0,12534, 33 veredictos del 2026-08-20). Cambiarlo obliga a rejuzgarlo o la comparación se rompe → **el instrumento se congela cuando el primer contendiente ya pasó por él** (`LM.97`); lo que sí se puede añadir después es una capa **encima**. 🎁 **EL REGALO DEL DÍA: el detector se detectó a sí mismo, DOS VECES en diez minutos.** `P22` nació roja porque los literales que buscaba estaban **en la línea que los buscaba**; se arregló partiendo las agujas, y la prueba que obliga al freno a **morder** (`LM.13`) necesitaba un cebo con la llamada dentro… que **también vive en el mismo archivo**. 🔑 **Un detector de texto sobre código no lee código: lee texto** — no sabe qué es una llamada y qué es la palabra «llamada» (`LM.99`), y la versión corta es: **la lista de patrones prohibidos es siempre el primer archivo que los contiene**. 📌 **`GUIDE.md` §6.e lleva ya la fila de `atribuidor.py`, en el mismo commit** — sexto archivo seguido que entra el día que nace. 📌 Y `LESSONS.md` suma `LM.96`, `LM.97`, `LM.98` y `LM.99`. ➡️ **La 111 sigue con `F.2`: los evals de los modos de falla propios del multi-agente** (`evals_orquestador.py`). ⭐ Y el README ya avisa de lo que NO hay que construir: **los 116 evals deterministas de `evals.py` del 5b siguen valiendo enteros**, porque los workers usan las mismas seis herramientas — lo que cambia al subir una capa **no es la herramienta, es quién decide llamarla**. 🎲 Y lo primero vuelve a ser sellar la apuesta y commitearla. 📎 Deudas nuevas de F.1: **falta el campo `origen` en `worker_inicio`** que diga si el encargo lo escribió el modelo o el harness (alta, no bloqueante — sin él no se puede distinguir un enrutado torcido de un experimento), **`orquestador` sin morder sobre datos reales** (alta, no bloqueante — se paga en F.3), **`numeros_publicados()` no ve cifras sin separador de miles** (baja, declarada en `P12`) y **el escalón 4 cruza los 33 veredictos con una traza PRESTADA de otra corrida** (baja, dicho en el informe). 📎 Siguen vivas las de E.2 —ningún módulo escribe `entorno` (alta), el latido sin cablear y sin quien lo escuche (media)—, las de E.1 —los cinco `open(REGISTRO, "a")` sin candado de disco (alta), `profundidad.py:594` con nombre fijo— y las de D.2, D.1 y el bloque C.)
