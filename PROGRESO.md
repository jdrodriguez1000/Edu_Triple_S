...
**Última actualización:** 2026-08-23 (sesión 107 — NIVEL 8: **EL BLOQUE D CIERRA. D.2 hecha: `skills_compartidas.py`, 28 pruebas, $0,000000 — segunda sesión seguida a cero.** 🎲 Primero se selló la apuesta y se commiteó sola, antes de la primera línea: **doce sesiones seguidas con ese orden**, y de las seis apuestas **una falló entera, una falló a medias y una falló en el número pero no en la dirección**. 🚨 **EL HALLAZGO DEL DÍA FUE MÍO Y LO CAZÓ UNA PRUEBA EN ROJO: escribí una comparación que solo podía dar UNA respuesta.** `coste_de_repartir()` devolvía `gana_compartir: False` — con 1 worker o con 20, con 1 vuelta o con 20, **con cualquier número**. No porque compartir perdiera: porque la cuenta **no podía decir otra cosa**. Faltaba el término que decide de verdad: **pedir una skill es una llamada a herramienta, o sea una vuelta entera de API**, y una vuelta más re-manda el prompt completo — **1 828 tokens medidos**. Con él puesto la cuenta se da la vuelta. 🔑 **Es `LM.66` en la capa del INSTRUMENTO: allá el defecto era un dato solo en su renglón, aquí es un cálculo solo en su renglón — y los dos dan el mismo verde que la versión correcta** → `LM.91`. La pregunta que lo caza no es *«¿está bien la cuenta?»* sino **«¿con qué entradas daría lo contrario?»**. 📌 Y `P16` no comprueba un resultado: comprueba que **la función produce las DOS respuestas** — la única de las 28 que la versión rota no habría pasado. 🔴 **LA APUESTA 1 GANÓ LA DIRECCIÓN Y PERDIÓ EL NÚMERO, Y LA UNIDAD BARATA ME DABA LA RAZÓN:** sellé «más de 1,4×»; en **caracteres** sale **1,48×** y habría cantado victoria, en **tokens** —que es lo que se factura— sale **1,34×** (5 598 contra 4 183). Los caracteres no se reparten igual entre dos textos: el menú es prosa larga, los cuerpos llevan listas y cifras → `LM.90`. ⭐ **Y el titular no es el 1,34×, es EL VUELCO: la 7ª llamada.** No hace falta un fan-out — **un solo agente con siete vueltas ya lo cruza**. *«El menú es el 14 % del total»* es verdad **por ficha** y mentira **por corrida**. 🔴 **LA APUESTA 4 FALLÓ EXACTAMENTE POR EL BORDE QUE SELLÉ CON ELLA:** este nivel corre en `claude-haiku-4-5`, cuyo mínimo de caché son **4 096 tokens**; el prefijo de un worker son **1 828**. **Faltan 2 268, y ni sumándole el menú entero llega.** 🚨 **El mínimo NO baja con la generación:** el mismo prefijo SÍ cachearía en Opus 5 (512) y NO en Haiku 4.5 — **el modelo más barato tiene el listón ocho veces más alto**. Y la API **no lo dice**: se acepta el `cache_control` y `cache_creation_input_tokens` vuelve en 0. Es `LM.15` con forma de descuento que no llegó. ✅ **LA APUESTA 5 SALIÓ EXACTA Y ES EL TITULAR DEL BLOQUE D ENTERO:** 12 hilos × 40 lecturas = **480 lecturas, 0 errores, 0 cuerpos equivocados**, sin un solo candado — donde D.1 perdía el **49,5 %** escribiendo. 🔑 **Lo compartido solo duele cuando alguien lo CAMBIA; cuando nadie lo cambia no duele, CUESTA** — y el coste cambió de sitio: esas 480 lecturas fueron **2 400 aperturas de archivo**, porque `leer_skill()` relee la carpeta entera para buscar un nombre. Nada se rompe, nada avisa, **y no sale en ninguna factura porque el disco no manda factura** → `LM.89`. 🟡 **La apuesta 3 acertó la mitad:** dije «el equilibrio es ≥ 2 y este fan-out nunca lo alcanza» — **es 3, y lo alcanza en el último escalón posible**. Y lo que decide no es el cuerpo de la skill (1 227 tok) sino **la vuelta extra de 1 828 que cuesta pedirla**. ✅ **La apuesta 2 ganó entera** y trae una primicia: recortar el menú por worker reproduce el `SOBRE.md` palabra por palabra — **primera vez que el aislamiento se mide en lo que el worker SABE, no en lo que puede HACER**. ✅ **La 6 también:** dos lecturas de la misma skill dan cuerpos distintos si el `.md` cambia entre medias, **las dos válidas y sin aviso** — `LM.66` en la capa del conocimiento, y `P23` deja la deuda escrita. 📌 **`GUIDE.md` §6.e lleva ya la fila de `skills_compartidas.py`, en el mismo commit** — tercer archivo seguido que entra el día que nace. ⚠️ **Y un error mío de proceso, dicho porque casi cuesta caro:** al actualizar este archivo con un script, lo abrí en modo escritura y el script murió a mitad — **`PROGRESO.md` quedó en 0 bytes**. Se recuperó entero de Git en un comando, y solo porque **el sobre se había commiteado al empezar**. La costumbre de las doce sesiones no era solo higiene de apuestas: era la copia de seguridad. ➡️ **La 108 abre el BLOQUE E — agentes programados**, la única parte del nivel **donde no hay nadie mirando la pantalla**, y su primera pregunta escrita es *«¿qué pasa si se dispara dos veces?»* — que es `LM.87` de ayer con fecha. 🎲 Y la primera cosa vuelve a ser sellar la apuesta y commitearla. 📎 Deudas nuevas de D.2, todas con dueño y ninguna bloqueante: **ningún worker lleva skills todavía** — D.2 midió la aritmética, que es donde estaba la sorpresa (media) —, **la huella de la skill no se anota en el registro** (media), el modelo de la apuesta 3 **supone que el cuerpo se lee en la primera vuelta** y si fuera la segunda el equilibrio se mueve (baja), y **`TOKENS_MEDIDOS` es una foto del 2026-08-23**: si cambia un `.md`, la tabla miente y nada lo comprueba (media). 📎 Siguen vivas las de D.1 —el candado rancio de 30 s sin medir, `cargar()` sin pedir el candado, `presupuesto.py:823` con dos candados al mismo archivo— y las del bloque C: el contrato de una CADENA (`LM.72`), `profundidad.py:213`, la apuesta 4 de C.6 sin medir de verdad, el nombre del archivo de registro con un solo modelo, `agente.costo()` viva en `router.py:298` y `supervisor.py:258`, y `recursion.py` sin pasar la capa hacia abajo.)
