...
**Última actualización:** 2026-08-25 (sesión 111 — NIVEL 8: **DOS PIEZAS EN UN DÍA: se CIERRA la deuda de F.1 (el campo `origen`) y CIERRA F.2 (`evals_orquestador.py`), 23 pruebas nuevas + 10 evals, $0,000000 — séptima sesión seguida a cero, y 8 de 8 apuestas enteras.** 🎲 Cada pieza selló su apuesta y la commiteó sola: **diecisiete sesiones seguidas** con ese orden. 🚨 **F.1 · LA DEUDA: EL HALLAZGO ESTABA EN SU ENUNCIADO Y APARECIÓ AL IR A PAGARLA.** Pedía *«un campo que diga si el encargo lo escribió el modelo o el harness»*, y **el modelo NO escribe encargos**: `orquestador.py:402` arma la frase en Python y el modelo solo aporta `monto` y `moneda` — **lo que elige es el DESTINO, no el texto**. Al pie de la letra habría parido un valor `"modelo"` que ninguna rama puede producir: el `capa` de la 97 por la puerta de atrás. 🔑 **Una deuda anotada es una HIPÓTESIS sobre el arreglo, no una especificación** (`LM.101`). 🔑 **Y el titular: el instrumento hoy SABE MENOS que ayer y por eso hoy acierta** — ayer el escalón 3 veía 3 encargos torcidos y decía `orquestador` (falso, los clavó C.2); hoy salen **`no_comprobable`**. El número empeoró y el instrumento mejoró: el campo se añade **hacia adelante**, los registros viejos no se reescriben (`LM.65`), y esa temporada de *«no consta»* **ES la evidencia de que faltaba** (`LM.100`). Hicieron falta **dos** estados: `no_atribuible` = *se sabe y no es de nadie*; `no_comprobable` = *no se sabe*. 🕵️ Defensa señalable: **`P28`-`P30` pasan la MISMA frase torcida tres veces cambiando solo `origen`** y exigen tres dueños; `P27` exige que la ausencia no salga verde; `P31` mata la errata antes de grabar. 📏 **F.2 — LOS EVALS DEL MULTI-AGENTE, Y SU VALOR NO SON LOS CASOS: ES EL CATÁLOGO.** El nivel ya llevaba **433 `check()`**, así que el trabajo no era escribir el 434: **19 modos de falla — 10 con eval propio, 5 medidos en otro archivo y NO duplicados, y 4 HUECOS declarados con su motivo.** ⚠️ **F.2 mide el DETECTOR, no el DEFECTO** (`LM.103`), y va escrito en la cabecera antes de la tabla: el enrutado torcido de la sesión 95 **no puede tener un eval determinista** porque quien elige el destino es el modelo — **diez verdes significan «cuando se tuerce, no pasa de aquí», NO «no se tuerce»**. Eso lo mide F.3, pagando. 💸 $0,00 demostrado y no prometido: `_ClienteTrampa` revienta si un caso llama al modelo y `P7` lo ve morder. 🚨 **`P9` es la prueba que vale: rompe la conducta de cada caso y exige que ESE caso se ponga rojo — 9 de 9 muerden.** Un eval que nace todo verde no ha demostrado nada. 🐛 **EL BICHO DEL DÍA ES MÍO Y ES `LM.20` POR QUINTA VEZ:** `evals_orquestador.py` escribió **3 `sin_trozo` en el registro PAGADO**, siendo un archivo que promete no tocarlo. `correr_evals()` desviaba el registro y **devolvía la ruta**, que ya no estaba enchufada porque la función restaura en su `finally`. 🔑 **Un `tmp` que sobrevive a su montaje parece válido: no da error y apunta a un archivo real que ya nadie mira.** → Muerto en el ORIGEN con `desviar()`, un `with` que es la **única** forma de obtener la ruta; retirado con medición al lado (134 → 131, ningún `llamada_api`). 🚨 **Y LO GORDO NO ES EL BICHO: EL PORTERO DE LA 97 NO LO CAZÓ, Y SU DOCSTRING DECÍA QUE SÍ** — prometía *«cualquier módulo, incluidos los que todavía no existen»* y recorría **una lista de cinco nombres**. 🔑 **Un portero que vigila una LISTA no vigila una clase: vigila esa lista, y una lista se queda vieja al día siguiente** (`LM.102`) — y la docstring **blindaba** el hueco. → Ahora se queja de lo que **no reconoce**: cualquier módulo con `_pruebas` sin vigilar ni descartar-con-razón. Con la lista de ayer devuelve `['evals_orquestador']`: **muerde sobre el caso real**. 📌 Hacen falta las **dos** listas: sin la de descartados, *«no vigilado»* y *«se nos olvidó»* se ven igual. ⚠️ De rebote, la prueba 8 de `traza.py` comparaba contra `len(corridos) == 5` y **se puso roja sin que nada estuviera mal** — el mismo defecto de `avisador.py`; la lista subió a constante `VIGILADOS`. 🚨 **HALLAZGO LATERAL — alta · NO bloqueante, y NO es de hoy:** `registro_pruebas_gratis.jsonl` tiene **una línea partida por dentro**, la 626, del **2026-08-24T19:19:15** — la cola de un `worker_fin` cuya cabeza se perdió entre dos corridas del mismo segundo. **`compartida.py:363` lo había escrito en la sesión 106 palabra por palabra:** `worker.py` y `orquestador.py` tienen **DOS objetos `Lock()` distintos** y `presupuesto.py:823` apunta los dos al mismo archivo — *dos cerraduras en dos puertas de la misma habitación*. **Se pisaron.** Hoy no bloquea porque los dos registros pagados son dos archivos distintos; **el día que una corrida PAGADA los mande al mismo archivo, la línea se parte, la corrida ya está pagada y `atribuidor._leer()` revienta sobre datos que no se pueden rehacer sin volver a pagar. Antes de F.3.** Es deuda de E.1. 🚨 **Y sigue viva la otra:** `avisador.py` **sale con código 1** y ya salía antes de tocar nada (comprobado con `git stash`): `P8`, `P9` y `P12` llevan clavados los números de la 109 y cuentan un registro gitignoreado que **crece en cada corrida gratis**, así que **no puede volver a estar verde nunca**. ➡️ **La 112 es `F.3`: se ABRE EL SOBRE del bloque 0 y se corre el duelo contra la línea base.** ⚠️ Es **la primera sesión que paga de verdad en siete**, y arrastra dos condiciones escritas: **el candado de disco antes de pagar**, y que `orquestador` como culpable —hoy en CERO sobre datos reales— es lo que F.3 puede por fin ver morder. 🎲 Y lo primero vuelve a ser sellar la apuesta y commitearla sola — **y en F.3 hay un paso más y es nuevo: la HORQUILLA DEL COSTE se sella antes de pagar**, como en el paso 4 de C.1 ($0,024-$0,030 sellados, $0,026390 medidos). Sin ella, *«costó lo que costó»* no se puede desmentir. 📌 **Todo esto queda escrito también en `08-avanzado/README.md`, en el bloque «🔒 ANTES DE ABRIR EL SOBRE», con la tabla de deudas vivas y sus dos marcas** — ahí es donde va a mirar quien abra F.3. 📎 Deudas: **el candado de disco de E.1** (alta, bloqueante para F.3), **`avisador.py` siempre en rojo** (alta, no bloqueante), **ningún módulo escribe `entorno`** (alta, de E.2), el latido sin cablear (media), `numeros_publicados()` ciego a cifras sin separador (baja) y `profundidad.py:594` con nombre fijo. ✅ **CERRADAS hoy:** la del campo `origen`, y F.2 entera.)
