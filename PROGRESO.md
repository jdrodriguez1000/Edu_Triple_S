...
**Última actualización:** 2026-08-24 (sesión 108 — NIVEL 8: **ABRE EL BLOQUE E y E.1 CIERRA ENTERA: `disparador.py`, 67 pruebas, $0,000000 — tercera sesión seguida a cero.** 🎲 Primero se selló la apuesta y se commiteó sola: **trece sesiones seguidas** con ese orden. Cuatro escalones, cada uno rompiendo al anterior. 🚨 **EL HALLAZGO DEL DÍA: `open(ruta, "a") ` NO ES ATÓMICO ENTRE PROCESOS EN WINDOWS, y lo cazó una prueba mía en rojo.** `P21` se escribió para comprobar que el coste sale al doble y se puso roja: al solaparse **faltaban renglones del registro**, con cero líneas rotas y cero excepciones. El experimento decisivo —dos procesos con renglones de tamaños distintos— dio **46 renglones de longitud imposible, todos de 178**, cero mixtos y cero huecos. 🔑 **La huella es el 178: 200 − 22. Son la COLA de un renglón al que otro proceso le escribió encima los 22 primeros bytes.** El renglón llegó al disco y luego lo pisaron. ⭐ **Y no se entrelazan: se PISAN — por eso con renglones del mismo largo, que es un `.jsonl` de verdad, el pisotón es EXACTAMENTE INVISIBLE.** Es `LM.66` en la capa del sistema de archivos. ⚠️ **Segunda vez que Windows cambia un resultado del nivel, y en el sentido CONTRARIO:** en `LM.87` negó un `os.replace()` y armó RUIDO (26 procesos caídos); aquí no garantiza lo que POSIX sí, y hace SILENCIO. 📌 Muerde en `orquestador.py:228`, `worker.py:483`, `router.py:192`, `supervisor.py:120` y `pipeline.py:176` — hoy NO, y está medido por qué: escriben dentro de un `threading.Lock` y el fan-out usa HILOS. Muerde el día que haya dos procesos, que es el bloque E. ⚠️ **Y la propia `P21` se puso roja sola media hora después de escribirla:** la pérdida no ocurre siempre (`[0,2,2,0]`, 5,0 %). 🔑 **Por eso un fallo así vive años: no se reproduce a la primera, y el que lo ve una vez concluye que se equivocó. Una prueba de algo probabilístico mide la TASA, no el suceso.** 🟡 **LA APUESTA 1 SE PARTIÓ: (a) ✅, (b) 🔴 — y el motivo del fallo es mejor que la apuesta.** Sellé que un trabajo de más de 30 s haría que el segundo rompiera el candado por rancio: **no pasa**, el que espera se rinde a los 5 s y la caducidad **solo se comprueba mientras se espera** — con espera (5) < caducidad (30) el rancio es INALCANZABLE en un solapamiento. 🚨 **El bicho de verdad es peor: subiendo la espera a 60, el segundo declara el candado abandonado CON SU DUEÑO VIVO, lo borra y entra. Los dos a la vez, 0 errores.** 🔑 **El fallo no lo dispara un trabajo largo: LO DISPARA SER MÁS PACIENTE** — subir la espera es justo lo que cualquiera haría para arreglar la fila anterior. 🚨 **Y el candado FUNCIONÓ y no sirvió de nada en la fila corta: el segundo espera su turno y hace el trabajo entero igual. UN CANDADO SERIALIZA; NO DEDUPLICA.** ✅ **La apuesta 3 salió entera y es la pieza del día:** la marca de «ya corrí» necesita el requisito OPUESTO al de `corrida` — el **turno**, que se repite a propósito porque describe la RANURA, no la ejecución. Con él, las tres filas que el candado no podía dan una sola corrida. 🔑 Y `cedio` y `ya_estaba` son estados distintos: **el candado se borra al soltarlo, la marca se queda.** 🚨 **SEGUNDO HALLAZGO, misma familia: la marca se guardaba como `2026-08-24T03:00.json` y en Windows los dos puntos SEPARAN UN FLUJO ALTERNO DE NTFS.** `dir /r`: un archivo de 0 bytes con el nombre cortado y 73 bytes escondidos en `…json:$DATA`. **Y el veneno es que FUNCIONABA** —`exists()` sí, `O_EXCL` sí, pruebas verdes—; lo único que fallaba era **listar**: `glob("*.json")` daba CERO con todas las marcas puestas. ⭐ **Un identificador que se usa como nombre de archivo tiene que pasar por una puerta.** 🔑 **NO EXISTE «EXACTAMENTE UNA VEZ»:** marcar ANTES = como mucho una vez (si se rompe, nadie lo reintenta nunca); marcar DESPUÉS = al menos una vez (los renglones a medias del primero se quedan). ⭐ La elección **es del negocio**, y hay que hacerla antes: **el que no elige ya eligió** — el escalón 1 marcaba «después» sin saberlo. ⭐ **Y por fin se entiende para qué sirve `CANDADO_RANCIO_S`: NO es para el solape —ahí es inalcanzable—, ES PARA EL CADÁVER.** El que muere no suelta el candado y el reintento cede sin llegar a mirar la marca. Eso ordena los tres números: la caducidad **mayor que el trabajo** y **menor que el hueco entre disparos**. Hoy 21 s · 30 s · 3600 s: **cuadra por casualidad, y ahora está escrito** — y con eso la deuda del candado rancio de la 106 deja de ser un número suelto. ✅ **La apuesta 4 salió con matiz:** «se disparó dos veces» se escribe contando corridas; **«no se disparó» NO se puede escribir con el registro, porque el que no corre no escribe.** 🔑 **Un registro solo prueba lo que SÍ pasó; para lo que no pasó hace falta algo escrito ANTES, y es de otra clase: el registro lo escribe el que trabaja, el calendario lo escribe el que prometió.** ⭐ Y son TRES estados, no dos: sin anotar los intentos, «vino y no pudo» se ve igual que «no vino» — `LM.88` por tercera vez en el día, y la deuda del escalón 2 queda pagada. 🟡 **LA APUESTA 5: la letra ✅, el espíritu 🔴.** Los diez módulos, solos y en dos procesos a la vez: **cero comprobaciones rojas en las dos columnas**. 🚨 **Pero `traza` reventó 1 de 2 procesos, y una suite que muere no reporta rojo: no reporta nada.** La causa es de casa: `profundidad.py:594` usa un nombre de archivo FIJO en la carpeta del nivel y dos procesos chocan con `PermissionError [Errno 13]`, la misma clase de error de `LM.87`. 🔑 **Aposté que nada se pondría rojo y acerté por el motivo equivocado: no es que aguanten, es que ni llegan a quejarse.** 📌 Y el medidor tuvo **tres defectos propios antes de dar un dato**, los tres cazados porque reventaba también corriendo SOLO. ✅ **La apuesta 6 salió y su trampa se cumplió:** $0,000000, y el coste del disparo doble sigue siendo **una multiplicación, no una medición** — dicho antes de poder usarlo como excusa. 📌 **`GUIDE.md` §6.e lleva ya la fila de `disparador.py`, en el mismo commit** — cuarto archivo seguido que entra el día que nace. ➡️ **La 109 sigue con E.2 — fallar sin público**, cuya frase ya está escrita: *un fallo mudo a las 3 a.m. no existe hasta la factura*. Y arranca donde E.1 dejó el hilo: el `MUDO` del escalón 4 está calculado, pero **nadie se entera de él** — falta quién avisa. 🎲 Y lo primero vuelve a ser sellar la apuesta y commitearla. 📎 Deudas nuevas de E.1: **`profundidad.py:594` con nombre de archivo fijo** (media), **`skills_compartidas.py:586` con el `reconfigure()` dentro de `__main__`** (baja), **los cinco `open(REGISTRO, "a")` del nivel sin candado de disco** (alta, no bloqueante), **el disparador sin cablear a ningún agente real** (media) y **el coste del disparo doble multiplicado en vez de medido** (media). 📎 Siguen vivas las de D.2 —ningún worker lleva skills, la huella de la skill sin anotar, el modelo de la apuesta 3, `TOKENS_MEDIDOS` como foto del 2026-08-23— y las de D.1 —`cargar()` sin pedir el candado, `presupuesto.py:823` con dos candados al mismo archivo— y las del bloque C.)
...
- **🚨 UNA PROMESA ESCRITA QUE DURA TRES LÍNEAS** (sesión 106, D.1, y es el hallazgo del
  día). `06b/memoria.py` dice con todas sus letras que un archivo dañado **no se borra**,
  *«porque es la única evidencia de qué pasó»*. `cargar_memoria()` la cumple: avisa,
  devuelve `[]`, no toca nada. Y `guardar_dato()`, **tres líneas más abajo**, la llama,
  recibe ese `[]`, le añade el dato nuevo y **reescribe el archivo entero**. Medido con
  tres datos sanos y el archivo cortado a la mitad: **una sola escritura** deja un archivo
  válido con un dato, la evidencia borrada, y devuelve **`(True, "guardado")`**.
  🔑 **Nadie mintió: las dos funciones hacen exactamente lo que dice su comentario, y las
  dos son correctas leídas solas. Lo que no existía era el comentario que las mirara
  juntas** — y esa contradicción no vive en ninguna de las dos, así que no hay dónde
  leerla. ⭐ La raíz es un valor de retorno con dos significados: `[]` quería decir *«no
  hay nada»* y también *«no pude leer»*. Es el par **(resultado, motivo)** —la idea del
  estudiante, que ya había pagado seis veces en ese mismo repo— **faltando justo en la
  función de al lado**. → `LM.85`, y `LM.20` otra vez: la corrección ya estaba escrita.
- **La trampa del temario, medida, y con su control al lado** (sesión 106). *«Dos workers
  escribiendo a la vez sobre el mismo archivo»* sale así: **pierden el 49,5 % de lo que
  escriben y dejan el archivo PERFECTAMENTE VÁLIDO** — 0 % roto, cero excepciones, cero
  avisos. A 12 hilos, 81,9 % perdido y 24,5 % de archivos rotos. Con el arreglo, **0/480**.
  🔑 **Un `.jsonl` roto grita; un estado pisado calla**, y por eso el candado de B.2 no se
  podía copiar tal cual: el registro **añade**, la memoria **lee-modifica-reescribe**.
  → `LM.86`. ⚠️ Y las columnas «JSON roto» y «quedó vacío» salieron **idénticas** en las
  cuatro filas (0, 0, 3, 49 sobre 200): no es casualidad, es el bicho de arriba —cada vez
  que la carrera rompe el archivo, la defensa prudente lo convierte en *«no había nada»*.
- **⭐ EL ARREGLO QUE PARECE BIEN HECHO PIERDE MÁS QUE NO HACER NADA** (sesión 106, apuesta
  3, y es el titular técnico). Una versión con candado de hilos + relectura dentro +
  escritura atómica —**todo lo que una revisión de código llamaría correcto**— pasa las 18
  pruebas de hilos sin despeinarse y **pierde el 80 % entre cinco procesos**, contra el
  76,7 % de no poner nada. Con candado **de archivo**: 0 %. Un `threading.Lock` es un
  objeto en la memoria de UN proceso. 🔑 **Ese modo de fallo no se ve leyendo el código ni
  corriendo las pruebas que cualquiera escribiría: solo se ve lanzando dos `python`.**
  → `LM.87`. Y tiene fecha: el bloque E son agentes programados, y su primera pregunta
  escrita es *«¿qué pasa si se dispara dos veces?»*.
- **Dos candados sobre un archivo son cero candados, y la apuesta que lo negaba falló**
  (sesión 106, apuesta 1). Aposté que con líneas cortas no pasaría nada: con **60 bytes se
  pierden 16 registros de 2400**; con un solo candado, **0 de 2400 en las tres tallas
  probadas**. ⭐ Y el síntoma dominante **no son líneas rotas, son líneas que desaparecen
  enteras** — con 60 bytes hubo **0 rotas y 16 perdidas**. El comentario de
  `orquestador.py:206` predice el ruidoso (*«dos líneas se entrelazan»*) y ocurre el mudo.
  📌 La fila de UN candado es lo que lo convierte en medición y no en anécdota: sin ella,
  *«se rompió»* no dice si fue el candado o que el experimento apretara de más.
- **🪤 «Atómico» no quiere decir «siempre se puede»** (sesión 106, y no lo aposté). En
  POSIX `os.replace()` renombra encima de un archivo abierto; en **Windows lo NIEGA** con
  `PermissionError [WinError 5]` si otro lo tiene abierto, **aunque sea leyéndolo**. Medido:
  **26 de 60 procesos caídos**, y los 16 clasificados eran ese error **sin una sola
  excepción de otra clase**. ⚠️ En Linux esa prueba habría estado verde para siempre.
- **⚠️ Y el tercer síntoma silenciado del día fue MÍO** (sesión 106 → `LM.88`). Al ponerle
  reintentos al `os.replace()`, la columna de **procesos caídos** pasó de **26/60 a 0/60**
  y la de pérdidas **no se movió**. El reintento era correcto —cubre al lector que no pide
  el candado— pero hay que decir lo que hizo: **se llevó el síntoma ruidoso y dejó intacto
  el silencioso.** 🔑 Un `0` en esa columna se lee como *«ya no pasa nada»*, y lo que pasa
  es que **ya no se oye**. Tercera vez en el día la misma forma, con tres dueños distintos.
- **La política de memoria de la sesión 18 no sobrevivió al segundo escritor** (sesión 106,
  apuesta 5, ganada). *«Solo el PERFIL, sale el más viejo»* era correcta con un escritor;
  con tres significa **que el worker más hablador desaloja al callado**, y eso no lo decidió
  nadie. Ahora cada dato lleva `quien` y el desalojo tiene **reserva**. 🔑 Y la reserva no
  se inventó: es **`RepartoDeEntrada` de C.2 con otro recurso** — que la misma forma sirva
  para dólares y para renglones de memoria es la señal de que era estructura. Su límite
  queda **probado** (`P13`): si los ocho son de ocho workers, no se puede cumplir y sale el
  más viejo. Prometía un reparto, no un milagro.
