...
**Última actualización:** 2026-08-25 (sesión 111 — NIVEL 8: **SE CIERRA LA DEUDA DE F.1: el campo `origen`, 13 pruebas nuevas, $0,000000 — sexta sesión seguida a cero, y 4 de 4 apuestas enteras.** 🎲 Primero se selló la apuesta y se commiteó sola: **dieciséis sesiones seguidas** con ese orden. 🚨 **EL HALLAZGO ESTABA EN EL ENUNCIADO DE LA DEUDA, Y APARECIÓ AL IR A PAGARLA:** decía *«un campo que diga si el encargo lo escribió el modelo o el harness»*, y **el modelo NO escribe encargos** — `orquestador.py:402` arma la frase en Python y lo único que el modelo aporta son `monto` y `moneda`. **Lo que el modelo elige es el DESTINO, no el texto.** Implementada al pie de la letra, la deuda habría parido un valor `"modelo"` que ninguna rama puede producir: un estado que sale en cero sin que nadie sepa si es porque no pasa o porque no se puede escribir — el `capa` de la sesión 97 volviendo por la puerta de atrás. 🔑 **Una deuda anotada es una HIPÓTESIS sobre cuál es el arreglo, no una especificación de él** (`LM.101`), y el primer paso al abrir una vieja no es implementarla: es volver a medir si sigue siendo verdad. 🔑 **EL TITULAR ES QUE EL INSTRUMENTO HOY SABE MENOS QUE AYER, Y POR ESO HOY ACIERTA:** ayer el escalón 3 veía 3 encargos que no correspondían a la tarea y decía **`orquestador`** — falso, hubo que ir a `presupuesto.py:406` a mano para desmentirlo, los había clavado C.2. Hoy esas mismas 3 salen **`no_comprobable`**. **El número empeoró y el instrumento mejoró.** ⭐ El campo se añade **hacia adelante** y los registros viejos no se reescriben (`LM.65`): esa temporada de *«no consta»* **ES la evidencia de que el campo faltaba**, y rellenarla con el valor más probable la borraría (`LM.100`). 🚨 **Y hubo que partir el estado en DOS, porque son dos cosas:** `no_atribuible` = *se sabe lo que pasó y no es de ninguna capa*; `no_comprobable` = *no se sabe, falta el dato*. Un auditor que llama verde a lo segundo miente por omisión; uno que lo llama `no_atribuible` miente peor: **dice que ya lo miró.** 🕵️ La defensa contra el sospechoso —*«el que escribe el campo decide qué significa y elige las torceduras»*— está en el código y se puede señalar: **`P28`-`P30` pasan LA MISMA frase torcida tres veces cambiando solo `origen`** y exigen tres dueños distintos (`experimento` / `orquestador` / `no_comprobable`); si el campo no cambiara nada, `P30` es roja. Y **`P27`** exige que la ausencia **no** salga verde. 💸 **$0,000000: el campo se decide ARRIBA, antes de bajar**, así que un espía en lugar de `correr_worker` basta para ver las dos ramas grabarse (`P30`-`P32` de `presupuesto.py`, que viven ahí porque `ENCARGOS_DESIGUALES` es de ahí). 📌 `origen` viaja en **`worker_inicio` Y `worker_fin`**, no solo donde pedía la deuda: el atribuidor lee `worker_fin`, y un campo que califica a otro tiene que estar donde esté el calificado. 📌 `P31` mata el bicho gemelo: un `origen="plantila"` con errata **revienta antes de grabar**, porque si no se leería como la ausencia que el campo vino a hacer visible. 📌 `P32` protege `LM.97`: esto es una capa **ENCIMA**, no una casilla 12 — las 11 del juicio congelado no se tocan. ✅ Los registros pagados siguen en 131 y 405 líneas, el portero lo confirma. 🚨 **HALLAZGO LATERAL — importancia alta · urgencia NO bloqueante, y NO es de hoy:** `avisador.py` sale con código 1, y **ya salía con 1 antes de tocar nada** (comprobado sobre el árbol limpio con `git stash`). `P8`, `P9` y `P12` llevan clavados los números de la sesión 109 —*1520 líneas, 163 avisos*— y los cuentan sobre `registro_pruebas_gratis.jsonl`, que **está en `.gitignore` y CRECE cada vez que alguien corre las pruebas gratis**. 🔑 Es una prueba que **no puede volver a estar verde nunca**, porque mide algo que aumenta solo — y el daño no es el rojo: es que **una suite siempre roja enseña a no mirar el rojo**. Se deja sin arreglar a propósito: congelar ese número o volverlo relativo es una decisión del bloque E, no de F. ➡️ **La 112 sigue con `F.2`: los evals de los modos de falla propios del multi-agente** (`evals_orquestador.py`), y lo primero vuelve a ser sellar la apuesta y commitearla sola. ⭐ El README ya avisa de lo que NO hay que construir: **los 116 evals deterministas de `evals.py` del 5b siguen valiendo enteros** — lo que cambia al subir una capa no es la herramienta, es quién decide llamarla. 📎 Deudas: **`avisador.py` siempre en rojo** (alta, no bloqueante, nueva y anterior a hoy), **`orquestador` sin morder sobre datos reales** (alta, no bloqueante — se paga en F.3), **ningún módulo escribe `entorno`** (alta, de E.2), **los cinco `open(REGISTRO, "a")` sin candado de disco** (alta, de E.1), el latido sin cablear (media), `numeros_publicados()` ciego a cifras sin separador (baja, declarada en `P12`) y `profundidad.py:594` con nombre fijo. ✅ **CERRADA hoy:** la del campo `origen`.)
