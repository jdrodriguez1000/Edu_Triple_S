...
**Última actualización:** 2026-08-25 (sesión 112 — NIVEL 8: **SE CIERRA LA DEUDA DEL CANDADO, la única que era BLOQUEANTE para F.3 — y el día lo pagó un error MÍO de $0,102085, que es el saldo real de la sesión.** 🎲 4 de 4 apuestas ganadas, **la horquilla del coste PERDIDA**, y la apuesta se selló y se commiteó sola: dieciocho sesiones seguidas con ese orden. 🔑 **EL HALLAZGO QUE JUSTIFICÓ EL DÍA ESTABA EN EL ENUNCIADO DE LA DEUDA, otra vez.** La deuda decía *«un candado por ARCHIVO, como `_CANDADO_HILOS`»* — y `_CANDADO_HILOS` es un `threading.Lock`, que **no cruza procesos**. Medido con líneas del tamaño real, 6000 escrituras: un proceso con dos candados parte **54** líneas; dos procesos parten **329**; el candado compartido cierra el primero y **deja el segundo intacto**. Hacían falta **los dos**, y el de disco ya estaba escrito en `compartida.py` desde la 106. 🚨 **Y LO QUE AMENAZA A F.3 NO ES LA LÍNEA ROTA: ES LA QUE DESAPARECE SIN RUIDO.** La rota grita (`JSONDecodeError`) y por eso se vio; la perdida calla. `P23` lo deja escrito para siempre: **0 malas y 65 perdidas de 3000** — el bicho delante y el detector en verde. El duelo se puntúa **contando eventos**, así que a una corrida pagada con eventos ausentes le sale un veredicto tranquilo y falso. 🐛 **`LM.104` — UN EXPERIMENTO CON DATOS MÁS LIMPIOS QUE LOS REALES ABSUELVE AL CULPABLE.** El primer montaje usó todas las líneas **del mismo tamaño** y dio **0 malas**, con el defecto exacto delante. La cola solo sobrevive cuando una corta pisa la cabeza de una larga. Casi cierro con una frase medida, publicable y falsa. 🚨🚨 **EL BICHO DEL DÍA ES MÍO Y NO ESTABA EN EL CÓDIGO — `LM.105`.** Para comprobar que el arreglo no rompía nada corrí `python <modulo>.py` sobre los 22 módulos. Tres pagan al arrancar: `pipeline.py`, `linea_base.py`, `juez_duelo.py`. **$2,520038 → $2,622123 = $0,102085**, y la horquilla sellada esa mañana decía $0,000000 con *«una sola `llamada_api` la tumba»*: hubo 6. Y lo caro no fue el dinero — **`linea_base.py` REHÍZO la línea base sellada el 20-ago (la contrincante del duelo) y `juez_duelo.py` REJUZGÓ sus 33 veredictos.** 🔑 **Ningún freno del nivel lo vio, y no por descuido: el portero de la 111 vigila «módulos con `_pruebas` que nadie clasificó», y los tres que cobran NO TIENEN PRUEBAS.** No se escaparon de la lista: **se escaparon del criterio.** «Lo que puede ensuciar» y «lo que puede cobrar» son dos clases que **no se tocan**, y el nivel entero vigilaba la primera. → Hacía falta **un segundo portero**: `traza.portero_del_dinero()`, más `compartida.exigir_pagar()` en el ORIGEN (la lección de la 97) con el parámetro **`tambien_pisa`**, porque hay un tercer daño que ni ensucia ni cobra: **sobrescribir algo sellado**. Y la otra mitad es `LM.20` por enésima vez — **`worker.py` tenía el freno puesto desde el bloque A**, en la misma carpeta, y tres archivos no lo alcanzaron. 🕵️ Visto morder: la prueba **48 señala los tres archivos de ayer sin su freno**, y `md5sum` confirma que ahora la corrida entera de los 22 módulos **no toca ni un byte** de los pagados ni de los sellados. 📏 **Medido antes de decidir, y corrige una afirmación mía falsa de esa mañana:** dije *«el duelo no mide lo que dice medir»* y **no es cierto** — las dos líneas base dan mediana **$0,023165** contra **$0,023006** (0,7 %), y la dispersión *dentro* del set sellado (10,0 s – 12,67 s) es **mayor** que la diferencia entre ambos. Restaurar no cambia el resultado del duelo; cambia que el artefacto que F.3 abra sea el que se selló. 📎 **`LM.106`, de rebote:** `P18` se puso roja sin que nada estuviera mal — su umbral (25 %) vive dentro de su propio ruido (27-50 %). Sospeché de mi cambio y los primeros números lo confirmaban (25-33 % contra 39-42 %); **interleavando** las dos versiones se deshizo (38/50/42 contra 36/32/27): lo que medía la primera tanda era **el orden de ejecución**, no el código. ➡️ **La 113 es `F.3`: se ABRE EL SOBRE.** El candado ya no la bloquea. ⚠️ Arrastra **una decisión suya sin tomar**: los 5 archivos contaminados siguen sin restaurar, con copia a salvo en el scratchpad. Y sigue en pie lo demás: la horquilla del coste se sella antes de pagar, `orquestador` como culpable está en CERO sobre datos reales, y `enrutado_torcido` es un HUECO que solo se ve pagando. 📌 Deudas vivas: **restaurar o no los 5 archivos** (media, antes de F.3), **`avisador.py` siempre en rojo** (alta, no bloqueante), **ningún módulo escribe `entorno`** (alta, de E.2), `P18` mal calibrada (media), el latido sin cablear (media), `numeros_publicados()` ciego a cifras sin separador (baja) y `profundidad.py:594` con nombre fijo. ✅ **CERRADA hoy: la deuda del candado de disco de E.1, que era la única BLOQUEANTE.**)
