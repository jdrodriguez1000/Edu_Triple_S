...
**Última actualización:** 2026-08-25 (sesión 113 — NIVEL 8: **SE ABRIÓ EL SOBRE. EL DUELO ESTÁ DECIDIDO Y B PIERDE — lo decide el TIEMPO, y el juez no hizo falta.** 🏆 B tardó **15,63 s** contra un tramo de **≤ 8,33 s**: **1,41× A**, cuando tenía que bajar a 0,75×. En **coste PASÓ** ($0,026790 contra un techo de $0,046388), y **aciertos quedó SIN MEDIR** a propósito: el sobre dice *«B gana solo si cumple los TRES»*, así que ninguna casilla podía darle la vuelta al tiempo y **pagar al juez habría afinado un número que no cambia el resultado**. 🚨 **Y EL SOBRE HABÍA PREDICHO EL PORQUÉ CINCO DÍAS ANTES:** su corrección del 20-ago decía *«A ya paraleliza… el margen que se le suponía al paralelo YA ESTABA COBRADO por el contendiente A»* — A pide las tres `tasa` en un solo turno. **B es más lento POR la capa de arriba, no a pesar de ella.** 🔑 De ahí `L8.1`, que corrige la apuesta del estudiante: *«independizable»* es solo la MITAD de la condición — **reparte cuando sea independizable Y una capa no pueda hacerlo en un turno; si cabe en un turno, la capa de arriba es coste puro.** ⚠️ La Parte 1-bis (expectativa de esta terminal) **FALLÓ**: dijo *«B algo más rápido»* y salió 41 % más lento; en coste acertó frente al estudiante (1,16×, no *«mucho más»*). 🐛 **HALLAZGO QUE NO VENÍA EN NINGUNA VARA — `L8.2`: B AFIRMÓ HABER GUARDADO EL REPORTE SIN TENER LA HERRAMIENTA.** 2 de 3 corridas, cero llamadas a `guardar_reporte`. La capa de arriba no ve lo de abajo, y cuando le falta una capacidad **no lo sabe: la narra.** 📌 Y su contrario también salió: el CAD se cayó en la corrida 1 y B **lo declaró** en vez de inventarlo — las causas de C.2 funcionando. 🔒 **`L8.3` — LA VENDA DEL JUEZ NO EXISTÍA.** La rúbrica la exigía desde el bloque 0 y `leer_corridas()` la documentaba, pero el registro del orquestador anota `consultar_moneda`: **el vocabulario de la primera línea le decía al juez que había dos capas.** Cinco semanas de decisión correcta y ni una línea que la cumpliera. → `duelo.py` y su **aplanador**, con 8 pruebas gratis sobre las 7 corridas ya pagadas del bloque C. ⚠️ **El B construido ≠ el B sellado**: el sobre le daba `guardar_reporte` al orquestador y no la tiene (arsenal 4 contra 2). **El veredicto aguanta por la DIRECCIÓN del error** (`L8.4`): esa herramienta añade una vuelta, o sea más lento y más caro — nunca más rápido, y B falló por 7,3 s. 🐛 **El bicho del día es mío y es el de la 111 otra vez:** `aplanar()` tenía un `else: pass` cuyo comentario decía *«la deja pasar en vez de tirarla en silencio»* y hacía lo contrario. Lo cazó **la prueba 7**, la que rompe la lista de frontera y exige que la venda se caiga. Reescrita en una sola pasada, más simple. 💰 **HORQUILLA PERDIDA POR ABAJO: $0,077903 contra $0,15–$0,30 sellados** — se selló contando con el juez y el juez no se corrió. **Se falla y se explica, no se reajusta** (`L8.5`). 🔴 **Y una CORRECCIÓN de la 112: costó $0,202977, no $0,102085.** Mi contador sumaba campos solapados (incluido `acumulado_usd`, un total corriendo) y **solo miraba los registros** — el juez, que fue el 58 % de la factura, escribe su coste en un `.json`. `LM.15` por tercera vez en un día. ✅ **BLOQUE F CERRADO Y BLOQUE G ESCRITO**, con la respuesta a la pregunta del nivel y las lecciones `L8.1`–`L8.5`. 📌 El sobre se cerró **sin retoques**: ni un tramo movido con el resultado delante. ➡️ **Lo siguiente es `METODO.md`**, la última tarea del recorrido. 📎 Vivas: B sin `guardar_reporte` (alta, no bloqueante), los aciertos de B sin medir (media), los 5 archivos de la 112 **se dejan sin restaurar** (decidido), `avisador.py` siempre en rojo (alta), ningún módulo escribe `entorno` (alta), `P18` mal calibrada (media) y el latido sin cablear (media).)
