...
**Última actualización:** 2026-08-23 (sesión 104 — NIVEL 8: **C.6 CERRADO — la ÚLTIMA pieza del bloque C, aunque el bloque no lo esté: a C.3 le sigue faltando su bloque escrito. C.6 medido, cinco apuestas de seis ganadas, y la sexta se deja SIN RESOLVER aunque el número la confirme. $0,058556.** 🔑 El hallazgo del día es de contabilidad y no de agentes: **el precio estaba pegado al MÓDULO, no a la llamada** — `agente.costo()` tarifa con constantes fijadas al importar, así que en cuanto las dos capas dejaron de compartir modelo, la contabilidad entera del nivel 8 siguió facturando a precio de haiku **sin una queja**. Medido: **5,0000000000× exacto**, y confirmado en una factura de verdad — $0,024075 arriba donde el harness de ayer habría dicho $0,004815. 🚨 **Y el hallazgo no es el factor: es que la mentira sale LIMPIA.** Los tres modelos del catálogo tienen la salida a 5× la entrada, así que tarifar mal **multiplica todo por una constante**: las partes siguen sumando el total, el árbol de C.1 sigue cuadrando con `auditar()`, y **todos los cuadres internos salen verdes porque todos usan la misma tabla mala.** Es `LM.66` del revés — allí los dos caminos eran independientes y uno pudo desmentir al otro; aquí comparten la fuente del error y **confirman la mentira en coro**. → `LM.82`. ✅ **Apuesta 2, 0 de 191 líneas**, y al buscar el porqué salió algo mejor: en C.1 el testigo ya estaba grabado **porque era la SALIDA de una herramienta**, y el registro guarda salidas; **el modelo es una ENTRADA de la petición**, y de la petición no se guardaba nada → `LM.83`. ✅ **Apuesta 3, las dos mitades:** el techo de C.2 no hay que tocarlo, y el motivo es **la unidad** —escrito en dólares, el precio ya va dentro del número—, pero se compara contra un gasto que salía de la tabla mala: **habría visto un 9 % donde había un 46 %**. Un freno correcto conectado a un instrumento ciego no es medio freno: es ninguno. 🎯 **Apuesta 5, TOTAL: 6 de 6.** Las seis corridas de dos capas ya pagadas y la de hoy con opus arriba tienen la misma forma de árbol, `((0,1),(1,3),(2,3))`, cero quejas. **El modelo de arriba cambió la factura y no cambió ni una decisión** — el README lo *afirmaba* y ahora está medido: $0,019 por corrida a cambio de nada. 🎁 **Y solo se pagó UNA corrida:** la de haiku estaba pagada desde la 97 con su árbol grabado, así que **comparar contra lo ya pagado fue la mitad del ahorro del día** — el interés de haber puesto C.1 entonces. ✅ **Apuesta 6:** $0,058451 dentro de la horquilla sellada $0,045–$0,060, y cuadró porque esta vez **también se selló el modo de fallo**. 🟡 **Apuesta 4: el número la confirma (0 % de ahorro) y se deja SIN RESOLVER a propósito** — un 0 % es compatible con *«llegó y no tenía nada que recortar»* y con *«llegó y no hizo nada»*, y el experimento no los separa. → `LM.84`. 🚨 **La trampa del `effort` sí se vio morder:** haiku devuelve **400** con el motivo escrito, y **un 400 no se factura** — la mitad mala salió gratis; lo que se paga es el **control**. ⚠️ **CUATRO errores míos, los cuatro medidos y dichos con el número al lado:** razoné en tokens y la factura es en dólares (la salida es 6,1 % de los tokens y **24,6 % del gasto**); **escribí una prueba que no podía fallar dentro del archivo donde se cuenta esa lección** (las 12 y 13 prometían ponerse rojas y no podían — describen el mundo de ayer, y el mundo de ayer nunca se pone rojo; entraron las 17-22 con cliente espía y **la 20 mata el agujero**); `ESFUERZOS` tenía cuatro valores y son **cinco** —faltaba `xhigh`, escrito de memoria y corregido por la documentación, y **el que faltaba era el útil**: un validador con la lista corta **no deja pasar basura, rechaza cosas buenas diciendo que son basura**—; y el comparador de árboles tomaba «hoy» como la última corrida **en orden de archivo**, señaló un árbol de un nodo y **no dio error** (`LM.15` con ropa de índice: el orden de un archivo no es el orden del tiempo). 🎁 **Y la corrección del 1b resultó ELLA MISMA imprecisa, y lo dijo una prueba en rojo:** se escribió que las 12 y 13 «describen el mundo de ayer y el mundo de ayer nunca se pone rojo», y la 13 se puso roja en cuanto la corrida pagada tocó el registro. Las dos rojas eran correctas — el archivo dejó de ser homogéneo, y la suposición de que todas las líneas eran de un modelo llevaba trece sesiones sin estar escrita en ninguna parte. ⭐ **El pasado no cambia; el archivo donde está guardado, sí.** Ahora se separan por la ausencia del campo `modelo` (`antes_de_c6`), y la 13 se partió en dos: el agujero medido y su tapa en renglones distintos. 📎 **Y `modelos.py` entró en la lista de `GUIDE.md` §6.e EL MISMO DÍA en que se escribió**, por primera vez: las dos veces anteriores la lista se completó *después* de que un archivo mordiera. 🔑 La lista no se mantiene revisándola — se mantiene **añadiendo la fila en el mismo commit que crea la bandera.** 📊 `modelos.py` nuevo con **23 pruebas** y 4 modos · `worker.py` y `orquestador.py` cableados con `Capa(modelo, esfuerzo)` **por la puerta**, `None` = la conducta de siempre y **las siete suites del nivel siguen verdes** · `LESSONS.md` 81 → **84** · el README del nivel gana los bloques de C.6 y la pieza queda ✅. ➡️ **La 105 arranca el BLOQUE D (lo compartido)**, y quedan cuatro deudas con dueño: la apuesta 4 sin medir de verdad (hace falta un turno donde el modelo RAZONE, no donde despache), el nombre del archivo de registro que sigue llevando un solo modelo, `agente.costo()` todavía viva en `router.py:298` y `supervisor.py:258`, y `recursion.py` sin pasar la capa hacia abajo. 📎 Y siguen abiertas las tres del bloque C: el contrato de una CADENA (`LM.72`), `profundidad.py:213`, y que **C.3 nunca tuvo su bloque escrito** en el README.)
